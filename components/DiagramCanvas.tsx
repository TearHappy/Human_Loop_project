...
const DEFAULT_NODE_TEXT = "#FFFFFF";
const DEFAULT_EDGE_COLOR = "#FFFFFF";
const SUBGRAPH_FILL = "#edf2f7";
const SUBGRAPH_STROKE = "#ffffffff";
const SUBGRAPH_LABEL_COLOR = "#ffffffff";
...
function computeSubgraphSeparationShift(
	...
	const overlapX =
		Math.min(moving.right, expanded.right) -
		Math.max(moving.left, expanded.left);
	const overlapY =
		Math.min(moving.bottom, expanded.bottom) -
		Math.max(moving.top, expanded.top);
...
function computeNodeAlignment(
	...
	if (verticalGuide) {
		const targetPoint = nodes.find(
			(entry) => entry[0] === verticalGuide.targetId,
		)?.[1];
		if (targetPoint) {
			const targetBox = createNodeBox(targetPoint);
			guides.vertical = {
				...verticalGuide,
				x: verticalGuide.kind === "center" ? finalBox.centerX : verticalGuide.x,
	...
	if (horizontalGuide) {
		const targetPoint = nodes.find(
			(entry) => entry[0] === horizontalGuide.targetId,
		)?.[1];
		if (targetPoint) {
			const targetBox = createNodeBox(targetPoint);
			guides.horizontal = {
	...
	const normalizedGuides =
		guides.vertical || guides.horizontal ? guides : EMPTY_GUIDES;
...
export default function DiagramCanvas({
		...
		setContextMenu((prev) => {
			if (prev.target) {
				if (prev.target.type === "node") {
					onDeleteNode(prev.target.id);
				} else {
					onDeleteEdge(prev.target.id);
				}
			}
	...
	const finalPositions = useMemo(() => {
		...
		for (const node of diagram.nodes) {
			const override =
				draftNodes[node.id] ?? node.overridePosition ?? node.renderedPosition;
	...
	const edges = useMemo<EdgeView[]>(() => {
		return diagram.edges
			.map((edge) => {
				...
				const overridePoints = baseOverrides.map((point) => ({
					x: point.x,
					y: point.y,
				}));
				...
				const route = hasDraftOverride
					? [{ x: from.x, y: from.y }, ...overridePoints, { x: to.x, y: to.y }]
					: renderedRoute;
					...
					if (handlePoints.length === 1) {
						...
						handlePoints.forEach((point, idx) => {
							const distance = Math.hypot(
								point.x - routeCentroid.x,
								point.y - routeCentroid.y,
							);
							if (distance < bestDistance) {
								bestDistance = distance;
								bestIndex = idx;
				...
				const labelPoint =
					labelHandleIndex !== null
						? { ...handlePoints[labelHandleIndex] }
						: labelCenterForRoute(route);
	...
	const fitBounds = useMemo(() => {
		...
		if (!Number.isFinite(minX)) {
			// Use larger default bounds to prevent flash on diagram load
			minX = -200;
			maxX = 200;
			minY = -150;
			maxY = 150;
	...
	useEffect(() => {
		// Skip animation on first bounds initialization to prevent flash
	...
	const getDiagramPointFromClient = (
		clientX: number,
		clientY: number,
	): Point | null => {
	...
	const handleSubgraphPointerDown = (
		id: string,
		...
		for (const edge of diagram.edges) {
			...
			edgeOverrides[edge.id] = baseOverride.map((point) => ({
				x: point.x,
				y: point.y,
			}));
		}
	...
	const handleEdgeDoubleClick = (
		...
		const diagramPoint = getDiagramPointFromClient(
			event.clientX,
			event.clientY,
		);
		if (!diagramPoint) {
	...
	const handlePointerMove = (event: ReactPointerEvent<SVGSVGElement>) => {
		...
		if (dragState.type === "node") {
			...
			const snappedPosition = {
				x: alignment.appliedX
					? alignment.position.x
					: snapToGrid(alignment.position.x),
				y: alignment.appliedY
					? alignment.position.y
					: snapToGrid(alignment.position.y),
			};
			...
			setAlignmentGuides((prev) =>
				guidesEqual(prev, EMPTY_GUIDES) ? prev : EMPTY_GUIDES,
			);
			setDragState({
				...dragState,
				delta: resolvedDelta,
			...
			if (Object.keys(dragState.edgeOverrides).length > 0) {
				setDraftEdges((prev: DraftEdges) => {
					...
					for (const [edgeId, basePoints] of Object.entries(
						dragState.edgeOverrides,
					)) {
	...
	const handlePointerUp = (event: ReactPointerEvent<SVGSVGElement>) => {
		...
		if (currentDrag.type === "node") {
			if (currentDrag.moved) {
				...
				const result =
					auto && current && isClose(current, auto) ? null : current;
			...
			if (currentDrag.moved) {
				const normalized = currentDrag.points.map((point: Point) => ({
					...point,
				}));
			...
			if (currentDrag.moved) {
				...
				for (const nodeId of currentDrag.members) {
					...
					const auto = node?.autoPosition;
					nodeUpdates[nodeId] =
						auto && isClose(finalPoint, auto) ? null : finalPoint;
				}

				for (const [edgeId, basePoints] of Object.entries(
					currentDrag.edgeOverrides,
				)) {
	...
	useEffect(() => {
		if (
			dragState &&
			(dragState.type === "node" || dragState.type === "subgraph")
		) {
			return;
		}
		setDraftNodes((prev: DraftNodes) => {
			...
			for (const [id, point] of Object.entries(prev)) {
				...
				const resolved =
					node.overridePosition ?? node.renderedPosition ?? node.autoPosition;
		...
		<div ref={wrapperRef} className="diagram-wrapper">
			...
			>
				...
				{subgraphViews.map((subgraph) => {
					...
					const effectiveTopLeft = {
						x: subgraph.x + offsetX,
						y: subgraph.y + offsetY,
					};
					...
					const dragging =
						dragState?.type === "subgraph" && dragState.id === subgraph.id;
					return (
						<g
					...
					return (
						...
						>
							...
							{handlePoints
								...
								.filter(
									({ index }) =>
										labelHandleIndex === null || index !== labelHandleIndex,
								)
								.map(({ point, index }) => {
				...
				{nodeEntries.map(([id, position]) => {
					...
					const defaultFill = SHAPE_COLORS[node.shape] ?? "#9CA3AF";
				...
				{verticalGuide
					? (() => {
							...
							const end = toScreen({ x: v.x, y: v.y2 });
							return (
								<line
									key="vertical-guide"
									className={`alignment-guide alignment-guide-vertical alignment-guide-${v.kind}`}
									x1={start.x}
									y1={start.y}
									x2={end.x}
									y2={end.y}
					...
					? (() => {
							...
							return (
								<line
									...
									className={`alignment-guide alignment-guide-horizontal alignment-guide-${h.kind}`}
									x1={start.x}
									y1={start.y}
									x2={end.x}
									y2={end.y}
