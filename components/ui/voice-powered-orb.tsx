...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
	...
	const lastVoiceLevelRef = useRef<number>(0);
  ...
  `;
	...
	// Voice analysis function - OPTIMIZED
	const analyzeAudio = useCallback(() => {
		if (!analyserRef.current || !dataArrayRef.current)
			return lastVoiceLevelRef.current;

		analyserRef.current.getByteFrequencyData(
			dataArrayRef.current as Uint8Array,
		);

		// Early exit: check first 8 samples for silence
		let quickCheck = 0;
		for (let i = 0; i < 8; i++) {
			quickCheck += dataArrayRef.current[i];
		}
		if (quickCheck < 40) {
			lastVoiceLevelRef.current = 0;
			return 0;
		}

		// OPTIMIZED: Calculate RMS only for voice frequencies (300Hz-3kHz)
		// With 16kHz sample rate and 256 FFT, bins 5-50 cover voice range
		let sum = 0;
		const voiceStart = 5; // ~300Hz
		const voiceEnd = 50; // ~3kHz
		const voiceBins = voiceEnd - voiceStart;
		...
		lastVoiceLevelRef.current = level;
		return level;
	}, [voiceSensitivity]);
		...
		try {
			...
			const stream = await navigator.mediaDevices.getUserMedia({
				audio: {
					...
					sampleRate: 16000, // OPTIMIZED: 16kHz is perfect for voice (was 44.1kHz)
				},
			});
			...
			analyserRef.current.fftSize = 256; // OPTIMIZED: 256 is perfect for voice (was 512)
	...
	useEffect(() => {
		...
		try {
			// OPTIMIZED: Cap DPR at 1.5 for massive GPU savings on retina displays
			...
			glContext = rendererInstance.gl as WebGL2RenderingContext;
			...
			const geometry = new Triangle(
				glContext as WebGLRenderingContext | WebGL2RenderingContext,
			);
			program = new Program(
				glContext as WebGLRenderingContext | WebGL2RenderingContext,
				{
			...
			let frameCount = 0;
			const AUDIO_THROTTLE = 4; // Analyze audio every 4 frames instead of every frame
			let lastFrameTime = 0;
			const TARGET_FPS_ACTIVE = 60;
			const TARGET_FPS_IDLE = 30; // OPTIMIZED: Reduce FPS when no voice
			...
			// OPTIMIZED: Page Visibility API - stop rendering when tab is hidden
			...
			const update = (t: number) => {
				...
				// OPTIMIZED: Skip rendering when tab is hidden
				if (!isVisibleRef.current) return;

				// OPTIMIZED: Adaptive FPS - 30fps when idle, 60fps when voice active
				const targetFPS =
					voiceLevel > 0.05 ? TARGET_FPS_ACTIVE : TARGET_FPS_IDLE;
				const frameInterval = 1000 / targetFPS;
				const timeSinceLastFrame = t - lastFrameTime;

				if (timeSinceLastFrame < frameInterval) return;
				lastFrameTime = t - (timeSinceLastFrame % frameInterval);
				...
				// OPTIMIZED: Throttle audio analysis to every 4th frame (15fps instead of 60fps)
				if (enableVoiceControl && isMicrophoneInitialized) {
					if (frameCount % AUDIO_THROTTLE === 0) {
	...
	}, [enableVoiceControl, initMicrophone, stopMicrophone]);
